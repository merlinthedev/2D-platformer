...
namespace GXPEngine.Core {
    ...
    public class GLContext {
        ...
        public void CreateWindow(int width, int height, bool fullScreen, bool vSync, int realWidth, int realHeight) {
            ...
            GL.glfwSetWindowTitle("2D Platformer");
            GL.glfwSwapInterval(vSync);

            GL.glfwSetKeyCallback(
                (int _key, int _mode) => {
                    bool press = (_mode == 1);
                    if (press) { keydown[_key] = true; anyKeyDown = true; keyPressedCount++; } else { keyup[_key] = true; keyPressedCount--; }
                    keys[_key] = press;
                });
            ...
            GL.glfwSetMouseButtonCallback(
                (int _button, int _mode) => {
                    bool press = (_mode == 1);
                    if (press) {
                        mousehits[_button] = true;
                    } else {
                        mouseup[_button] = true;
                    }
