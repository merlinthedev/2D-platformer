    ...
    public class MouseHandler {
        ...
        void HandleOnStep() {
            ...
            if (isOnTarget && !_wasOnTarget) {
                if (OnMouseOverTarget != null) {
                    OnMouseOverTarget(_target, MouseEventType.MouseOverTarget);
                }
            } else if (!isOnTarget && _wasOnTarget) {
                if (OnMouseOffTarget != null) {
                    OnMouseOffTarget(_target, MouseEventType.MouseOffTarget);
                }
            }
            ...
            if (!_wasMouseDown && Input.GetMouseButton(0)) {
                if (OnMouseDown != null) {
                    OnMouseDown(_target, MouseEventType.MouseDown);
                }

                if (isOnTarget && OnMouseDownOnTarget != null) {
                    OnMouseDownOnTarget(_target, MouseEventType.MouseDownOnTarget);
                }
                ...
                if (OnMouseUp != null) {
                    OnMouseUp(_target, MouseEventType.MouseUp);
                }

                if (isOnTarget && OnMouseUpOnTarget != null) {
                    OnMouseUpOnTarget(_target, MouseEventType.MouseUpOnTarget);
                }

                if (isOnTarget && _wasMouseDownOnTarget && OnMouseClick != null) {
                    OnMouseClick(_target, MouseEventType.MouseClick);
                }
            ...
            if (_lastX != Input.mouseX || _lastY != Input.mouseY) {
                ...
                if (isOnTarget && OnMouseMoveOnTarget != null) {
                    OnMouseMoveOnTarget(_target, MouseEventType.MouseMoveOnTarget);
                }
            }
        ...
        public Vector2 offsetToTarget => _offset;
