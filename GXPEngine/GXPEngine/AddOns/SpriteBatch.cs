            ...
            foreach (GameObject child in children) {
                if (child is Sprite) {
                    ...
                    for (int corner = 0; corner < 4; corner++) {
                        ...
                        if (bounds[corner].x < boundsMinX) {
                            boundsMinX = bounds[corner].x;
                        }

                        if (bounds[corner].x > boundsMaxX) {
                            boundsMaxX = bounds[corner].x;
                        }

                        if (bounds[corner].y < boundsMinY) {
                            boundsMinY = bounds[corner].y;
                        }

                        if (bounds[corner].y > boundsMaxY) {
                            boundsMaxY = bounds[corner].y;
                        }
                    }
                    tile.Destroy();
                }
